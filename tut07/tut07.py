...
def mini(contemp,dictionary_sub,f,rollno):
			...
			for z in dictionary_sub:
				...
				if z in f:
					f.remove(z)
			if len(f)!=0:		
				if rollno in output_dictionary:
					output_dictionary[rollno].append(f)
				else:
					output_dictionary[rollno]=[]
					output_dictionary[rollno].append(f)
				return rollno
...
feedback_not_submitted()  #Calling required function